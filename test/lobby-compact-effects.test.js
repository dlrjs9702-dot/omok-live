...
test('all ten games expose one shared rules selector with the original full rule text', () => {
  ...
  const selectedIds = [...html.matchAll(/<option value="(omok|omok2v2|connect4|yut|bingo|dots|cityking|othello|baseball|pictionary|liar)">/g)].map(m => m[1]);
...
test('announcement rows are compact with inline controls and game choice heights are condensed', () => {
  ...
  assert.match(html, /styles\.css\?v=1\.6\.24/);
