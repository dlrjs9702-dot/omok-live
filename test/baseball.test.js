...
test('number baseball is registered alongside existing games', () => {
  ...
  assert.deepEqual(listGames().map(x => x.id).sort(), ['baseball','bingo','cityking','connect4','davinci','dots','gostop','halligalli','liar','marathon','oldmaid', 'omok','omok2v2','othello','pictionary','twentyquestions','yut']);
});
