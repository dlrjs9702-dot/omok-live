...
test('two-player baseball API keeps secrets private, validates roles and resets the next round', { timeout: 25000 }, async t => {
  ...
  const created = await req('/api/rooms', host, { gameType: 'baseball', title: '4자리 승부', digitCount: 4 });
  assert.equal(created.status, 201);
  assert.equal(created.data.state.gameType, 'baseball');
  assert.equal(created.data.state.title, '4자리 승부');
  assert.equal(created.data.state.game.digitCount, 4);
  ...
  assert.equal((await req('/api/room/set-secret', host, { secret: '0123' })).status, 400);
  assert.equal((await req('/api/room/set-secret', host, { secret: '1234' })).status, 200);
  assert.equal((await req('/api/room/set-secret', host, { secret: '4567' })).status, 409);
  ...
  const both = await req('/api/room/set-secret', challenger, { secret: '4567' });
  assert.equal(both.data.state.me.mySecret, '4567');
  assert.equal(both.data.state.game.status, 'playing');
  assert.equal(both.data.state.game.turn, 'black');
  assert.equal((await req('/api/room/guess', challenger, { guess: '1234' })).status, 409);
  assert.equal((await req('/api/room/guess', host, { guess: '1123' })).status, 400);
  assert.equal((await req('/api/room/move', host, { x: 0, y: 0 })).status, 400);
  const first = await req('/api/room/guess', host, { guess: '4123' });
  assert.equal(first.status, 200);
  assert.equal(first.data.state.game.guesses[0].strikes, 1);
  assert.equal(first.data.state.game.turn, 'white');
  const second = await req('/api/room/guess', challenger, { guess: '1324' });
  assert.equal(second.status, 200);
  assert.deepEqual([second.data.state.game.guesses[1].strikes, second.data.state.game.guesses[1].balls], [2, 2]);
  const victory = await req('/api/room/guess', host, { guess: '4567' });
