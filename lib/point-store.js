...
function validateSettlement(raw) {
  ...
  return { settlementId: raw.settlementId, matchId: raw.matchId ? String(raw.matchId).slice(0, 200) : null, gameType: raw.gameType, transfers, summary, match };
}
...
class PostgresPointStore {
  ...
  async settle(raw) {
    ...
    return this.#tx(async (client) => {
      ...
      const record = { settlementId: plan.settlementId, matchId: plan.matchId, gameType: plan.gameType, at: new Date().toISOString(), transfers: results, summary: plan.summary, balances: current };
