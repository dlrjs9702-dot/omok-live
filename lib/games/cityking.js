...
const metadata = {
  ...
  name: '랜드킹',
  size: 24,
  rules: '독자 규칙의 2인 도시 보드게임. 주사위 두 개만큼 이동해 도시를 매입하고 상대 도시에 도착하면 개발 단계에 따른 통행료를 냅니다. 자기 소유 도시에 도착할 때 매입가의 절반을 내고 별장·빌딩·호텔을 한 단계씩 지을 수 있습니다. 출발을 지날 때마다 +200을 받고, 이벤트·공동기금 칸의 효과를 적용합니다. 더블이면 해당 칸의 매입·건설 등 처리를 마친 뒤 계속 굴립니다. 양쪽 플레이어의 차례가 모두 끝날 때 전체 턴이 1 증가하며, 상대를 파산시키거나 50턴 뒤 순자산이 높은 쪽이 승리합니다.',
};
...
function buildProperty(game, color, at) {
  if (!validColor(color)) return { legal: false, reason: 'not-player' };
  if (game.status !== 'playing') return { legal: false, reason: 'not-playing' };
  if (game.turn !== color) return { legal: false, reason: 'not-your-turn' };
...
module.exports = { ...metadata, TILES, START_CASH, START_BONUS, TURN_LIMIT, create, start, reset, rollDice, buyProperty, skipProperty, buildProperty, skipBuild, propertyLevel, propertyToll, buildCost, applyMove, publicState, moveError, netWorth };
