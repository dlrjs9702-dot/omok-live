...
function create(options = {}) {
  return { status: 'selecting', round: options.round || 1, revision: options.revision || 0,
    ...
    deadlineAt: null, winner: null, eliminationOrder: [], moveCount: 0, history: [] };
}
...
function publicState(game) {
  const { hands, pile, drawn, ...rest } = game;
...
const moveError = reason => ({ started: '이미 시작했습니다.', players: '2~4명이 필요합니다.', turn: '차례가 아닙니다.', stale: '화면이 갱신됐습니다.', phase: '현재 단계에서 할 수 없습니다.', target: '유효한 상대를 골라 주세요.', tile: '유효한 타일과 숫자를 골라 주세요.' })[reason] || '행동할 수 없습니다.';
module.exports = { id: 'davinci', name: '다빈치 코드', size: 4, rules: '흑·백 타일의 숨겨진 숫자를 추리해 마지막까지 살아남으세요.', create, reset, start, guess, stop, reveal, tick, publicState, tilesFor, drawnFor, moveError };
