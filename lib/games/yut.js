...
const metadata = {
  ...
  name: '윷놀이',
  size: 0,
  rules: '2인 윷놀이. 각자 말 4개를 모두 먼저 완주하면 승리합니다. 도·개·걸·윷·모만큼 이동하며 윷·모가 나오거나 상대 말을 잡으면 한 번 더 던집니다. 빽도가 나오면 보드 위의 말 하나를 한 칸 뒤로 물립니다(대기 중인 말은 낼 수 없고, 물릴 말이 없으면 차례가 넘어갑니다). 출발 직후 첫 번째 칸에 있는 말이 빽도로 물러나면 한 바퀴를 돌아온 것과 같이 완주 직전 칸으로 들어갑니다. 같은 편 말이 한 칸에 만나면 업어서 함께 움직입니다. 모서리에 정확히 멈춘 말은 다음 이동부터 지름길을 이용하며, 중앙에 정확히 멈춘 말은 항상 10자리 쪽 지름길로 출발합니다. 완주 직전 칸에 정확히 멈춘 말은 그 칸에 머무르며 다음 이동에서 한 칸 이상 더 나아가야 완주하지만, 그 칸을 지나칠 만큼 넉넉한 눈이 나오면 그 던지기 안에서 곧바로 완주합니다.',
};
...
// `path` on every destination below is the ordered list of nodes the piece actually visits,
// starting with its current position and ending with the returned position (or FINISH_LINE for a
// finish) -- the exact route this same walk already computes and used to throw away, now kept so
// the client can animate a piece hopping node-by-node instead of teleporting straight to the
// result. It is server-authoritative (the real judged path, including shortcut/center-junction
// anchoring), never guessed or recomputed client-side.
function forwardDestination(piece, steps) {
  ...
  let cursor = at;
  const visited = [piece.position];
  for (let remaining = steps; remaining > 0; remaining -= 1) {
    cursor = Math.min(cursor + 1, path.length - 1);
    visited.push(path[cursor]);
    // Landing exactly on the finish line (no steps left over in this same throw) rests there --
    // a separate later move is what actually finishes it. But a throw that reaches the finish
    // line with steps still to spare has overshot it, so the walk keeps going into 'finished'
    // instead of stopping (remaining === 1 means this step is the last one in the throw).
    if (path[cursor] === FINISH_LINE && remaining === 1) break;
  }
  const next = path[cursor];
  return next === 'finished'
    ? { status: 'finished', position: null, route, path: visited }
    : { status: 'board', position: next, route, path: visited };
}
...
//
// House rule (v1.6.67): a piece sitting on the very first outer cell (1) that gets backed off it
// doesn't stop at the start corner (0) -- on the physical board, the start corner and the finish
// line are the same landing spot approached from opposite directions, so retreating off cell 1 is
// treated exactly like having come all the way around: the piece rests on the finish line, same as
// any other way of reaching it (it still needs a further move to actually finish, same as always).
function backwardDestination(piece, backSteps) {
  if (!piece || piece.status !== 'board') return null;
  if (piece.position === 1) return { status: 'board', position: FINISH_LINE, route: 'outer', path: [1, FINISH_LINE] };
  ...
  const visited = [];
  for (let i = at; i >= cursor; i -= 1) visited.push(path[i]);
  return { status: 'board', position: path[cursor], route, path: visited };
}
...
function destination(piece, steps) {
  ...
  if (piece.status === 'home') {
    // Entering the board: there's no on-board "current position" to walk from, so the path is the
    // opening stretch of the outer route from the start corner (0) up to the landing spot.
    return { status: 'board', position: steps, route: 'outer', path: routePath('outer').slice(0, steps + 1) };
  }
