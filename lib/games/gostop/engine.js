...
// Capturing the whole stack of a month also takes any bonus card buried with a 뻑 on it (and, as
// with any bonus card acquired, one 피 from each opponent per bonus card).
function captureStack(ctx, game, month) {
  const bonus = game.floorBonus[month] || [];
  delete game.floorBonus[month];
  ctx.captured.push(...bonus);
  ctx.steal += bonus.length;
...
function stealFromEach(game, seat, times) {
  const stolen = [];
...
// Public card movements of one turn, in order, for the client's cosmetic animation only: every card
// named here is already face up (played, flipped, on the floor or in a captured pile).
function stealSteps(stolen, to) { return stolen.map(item => ({ k: 'steal', from: item.from, to, card: item.id })); }
...
function play(game, seat, cardId, { shake = false, bomb = false, kong = false } = {}) {
  ...
  if (isBonus(cardId)) {
    ...
    game.moveCount += 1;
    // 대박맞고 피뺏기: acquiring a bonus card takes one 피 from every opponent.
    const stolen = stealFromEach(game, seat, 1);
    const steps = [{ k: 'play', seat, cards: [cardId] }, { k: 'capture', seat, cards: [cardId] }, ...stealSteps(stolen, seat)];
    event(game, seat, ['bonus'], { played: [cardId], captured: [cardId], stolen, steps, turn: game.moveCount });
    // Nothing left to play (empty hand, empty deck, no bomb flips): the turn ends here, and the
    // bonus pi itself may have lifted the score to a go/stop decision.
    if (!game.hands[seat].length && !(game.bombFlips[seat] > 0)) {
      game.ctx = { seat };
  ...
  const ctx = { seat, month, played: [], held: [], placed: false, wasPair: false, captured: [], steal: 0, tags: [], revealed: [], bonusFlipped: [], flipped: null, steps: [] };
  game.ctx = ctx;
  game.moveCount += 1;
  ctx.turnId = game.moveCount;
  ...
  if (bomb || kong) {
    ...
    ctx.played = cards;
    ctx.steps.push({ k: 'play', seat, cards }, { k: 'match', cards: [...cards, ...matches] });
  ...
  if (shake) {
    ...
    ctx.steps.push({ k: 'reveal', seat, cards: [...ctx.revealed] });
  }
  game.hands[seat] = hand.filter(id => id !== cardId);
  ctx.played = [cardId];
  ctx.steps.push({ k: 'play', seat, cards: [cardId] });
  if (matches.length === 0) {
    ctx.placed = true;
    game.floor.push(cardId);
    ctx.steps.push({ k: 'place', cards: [cardId] });
    return flip(game);
  }
  if (matches.length === 1) {
    takeFloor(game, matches);
    ctx.held = [cardId, matches[0]];
    ctx.steps.push({ k: 'match', cards: [cardId, matches[0]] });
    return flip(game);
  }
  if (matches.length === 2) {
    ctx.wasPair = true;
    ...
    event(game, seat, ['choose'], { played: [cardId], options: matches, revealed: [...ctx.revealed], steps: [...ctx.steps], turn: ctx.turnId });
    return { legal: true, choice: 'floor' };
  }
  // Three of the month already on the floor (a 뻑 stack or an opening triple): take all four.
  takeFloor(game, matches);
  ctx.steps.push({ k: 'match', cards: [cardId, ...matches] });
...
function chooseFromPair(game, chosen) {
  ...
  ctx.steps.push({ k: 'match', cards: [ctx.played[0], chosen] });
  ctx.options = null;
  game.phase = 'play';
  return flip(game);
}
...
function flipOnly(game, seat) {
  ...
  game.moveCount += 1;
  game.ctx = { seat, month: null, played: [], held: [], placed: false, wasPair: false, captured: [], steal: 0, tags: ['bombFlip'], revealed: [], bonusFlipped: [], flipped: null, steps: [], turnId: game.moveCount };
  return flip(game);
}
...
function flip(game) {
  ...
  while (game.deck.length) {
    ...
    if (isBonus(next)) { ctx.bonusFlipped.push(next); ctx.steps.push({ k: 'flip', card: next, bonus: true }); continue; } // take it and flip again
    drawn = next;
    break;
  }
  ctx.flipped = drawn;
  if (drawn) ctx.steps.push({ k: 'flip', card: drawn });
  const M = ctx.month;
  ...
  if (ctx.held.length && N === M && !ctx.wasPair) {
    ...
    ctx.steps.push({ k: 'stack', month: M, cards: [...ctx.held, drawn, ...ctx.bonusFlipped] });
  ...
  if (ctx.placed && drawn && N === M) {
    ...
    ctx.steps.push({ k: 'match', cards: [ctx.played[0], drawn] });
  ...
  if (ctx.wasPair && drawn && N === M) {
    ...
    takeFloor(game, rest);
    ctx.steps.push({ k: 'match', cards: [drawn, ...rest] });
...
function resolveFlipped(game, drawn) {
  ...
  const matches = floorOfMonth(game, month);
  if (matches.length === 0) { game.floor.push(drawn); ctx.steps.push({ k: 'place', cards: [drawn] }); return finishTurn(game); }
  if (matches.length === 1) { takeFloor(game, matches); ctx.steps.push({ k: 'match', cards: [drawn, matches[0]] }); ctx.captured.push(drawn, matches[0]); return finishTurn(game); }
  if (matches.length === 2) {
    if (equivalent(matches[0], matches[1])) {
      takeFloor(game, [matches[0]]); ctx.steps.push({ k: 'match', cards: [drawn, matches[0]] }); ctx.captured.push(drawn, matches[0]); return finishTurn(game);
    }
    ctx.flipOptions = matches;
    game.phase = 'choose-flip';
    event(game, ctx.seat, ['choose'], { played: ctx.played, flipped: drawn, options: matches, revealed: [...ctx.revealed], steps: [...ctx.steps], turn: ctx.turnId });
    return { legal: true, choice: 'flip' };
  }
  takeFloor(game, matches);
  ctx.steps.push({ k: 'match', cards: [drawn, ...matches] });
...
function chooseFlip(game, seat, cardId) {
  ...
  ctx.steps.push({ k: 'match', cards: [ctx.flipped, cardId] });
...
function finishTurn(game) {
  ...
  // A bonus card flipped from the deck and kept (not buried in a 뻑) takes one 피 from each opponent.
  if (ctx.bonusFlipped.length) { ctx.steal += ctx.bonusFlipped.length; ctx.tags.push('bonus'); }
  // 판쓸이: the floor is left empty -- not with the player's own last card (막판 쓸 is not counted).
  const lastCard = game.hands[seat].length === 0 && !(game.bombFlips[seat] > 0);
  if (game.floor.length === 0 && !lastCard) { ctx.steal += 1; ctx.tags.push('sweep'); }
  const stolen = stealFromEach(game, seat, ctx.steal);
  const gained = [...ctx.captured, ...ctx.bonusFlipped];
  const steps = [...ctx.steps, ...(gained.length ? [{ k: 'capture', seat, cards: gained }] : []), ...stealSteps(stolen, seat)];
  event(game, seat, ctx.tags.length ? ctx.tags : ['play'], {
    played: ctx.played, flipped: ctx.flipped, captured: gained, stolen, revealed: ctx.revealed, steps, turn: ctx.turnId,
  });
...
function computeResult(game, winner, reason) {
  ...
  // 총통·3뻑 end the hand at a fixed 10점: only a carried-over 나가리 multiplier still applies.
  const instant = reason === 'chongtong' || reason === 'samppeok';
  const base = instant ? scoring.INSTANT_WIN_SCORE : winnerBreak.total;
  const goCount = instant ? 0 : game.goCount[winner] || 0;
  ...
  const losers = opponents(game, winner).map((seat) => {
    ...
    const bakList = instant ? [] : scoring.baks({ mode: game.mode, winner: winnerBreak, loser: loserBreak, loserWent: (game.goCount[seat] || 0) > 0 });
    const pay = scoring.payment({ score, goCount, shakes: instant ? 0 : game.shakes[winner] || 0, bombs: instant ? 0 : game.bombs[winner] || 0,
      nagari: game.nagariStreak || 0, bakList, pointsPerScore: game.pointsPerScore });
    return { seat, baks: bakList, went: game.goCount[seat] || 0, ...pay };
  });
  return { kind: 'win', winner, reason, instant, base, goCount, score, items: instant ? [] : winnerBreak.items,
    nagariStreak: game.nagariStreak || 0, pointsPerScore: game.pointsPerScore, losers };
}
...
function publicState(game) {
  ...
  return {
    ...
    choice: game.phase === 'choose-floor' ? { kind: 'floor', played: ctx?.played?.[0] || null, options: [...(ctx?.options || [])] }
      // Cards already taken this turn but not yet in a pile (the hand card and its match, a bomb's
      // four) are face up; the table keeps showing them beside the flipped card while it waits.
      : game.phase === 'choose-flip' ? { kind: 'flip', flipped: ctx?.flipped || null, options: [...(ctx?.flipOptions || [])], pending: [...(ctx?.held || []), ...(ctx?.captured || []), ...(ctx?.bonusFlipped || [])] }
      : null,
