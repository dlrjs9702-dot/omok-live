...
const INSTANT_WIN_SCORE = 10; // 총통, 3뻑: a fixed 10점 ("무조건 10점") -- no go points, go/shake/bomb multipliers or 박
const MEONGTTA_ANIMALS = 7; // 멍따 (shown as 멍박): winning with 7+ 열끗 doubles every loser's payment
...
function scoreBreakdown(capturedIds, gukjinAsPi = false) {
  ...
  return {
    ...
    gwangScore, animalScore, piScore, meongtta: t.animals.length >= MEONGTTA_ANIMALS,
  };
}

// 박 against one loser, judged from the winner's scoring categories and that loser's own pile
// (3인: every loser separately).
// - 피박: the winner scores with 피 and the loser holds 1..PIBAK_MAX 피 (by value); 0 피 is exempt.
// - 광박: the winner scores with 광 and the loser holds no 광 at all (비광 counts as a 광).
// - 멍박 (멍따): the winner holds 7+ 열끗; it does not depend on the loser's pile. The extra ×2
//   some rule sets add when the loser has no 열끗 at all is not applied (unconfirmed).
// - 고박: this loser had called 고 and someone else won.
function baks({ mode, winner, loser, loserWent }) {
  ...
  if (winner.meongtta && winner.animalScore > 0) list.push('meongbak');
...
// The per-loser payment: (기본 점수 + 고 점수) × 고 배수 × 흔들기 × 폭탄 × 나가리 × 박, then × 점당 P.
// Integers only; every factor is a power of two. `score` already includes the +N go points.
...
module.exports = { STOP_THRESHOLD, PIBAK_MAX, INSTANT_WIN_SCORE, MEONGTTA_ANIMALS, tally, scoreBreakdown, baks, goMultiplier, payment };
