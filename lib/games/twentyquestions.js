...
function regularActionsUsed(game) {
  const guesses = game.guessHistory.filter(guess => !guess.final).length;
  const pendingQuestion = game.pendingQuestion ? 1 : 0;
  const pendingGuess = game.pendingGuess && !game.pendingGuess.final ? 1 : 0;
  return game.questions.length + guesses + pendingQuestion + pendingGuess;
}
function beginFinalGuesses(game) {
...
function skipTurn(game, seat) {
  const error = canTakeTurn(game, seat);
  if (error) return { legal: false, reason: error };
  if (game.phase === 'final-guesses') {
    game.finalGuessIndex += 1;
    game.moveCount += 1;
    if (game.finalGuessIndex === game.finalGuessSeats.length) return finishRound(game, false);
    return { legal: true, skipped: true, final: true, nextSeat: currentTurn(game) };
  }
  game.turnIndex = (game.turnIndex + 1) % game.challengerSeats.length;
  game.moveCount += 1;
  return { legal: true, skipped: true, final: false, nextSeat: currentTurn(game) };
}
function submitQuestion(game, seat, question) {
  const error = canTakeTurn(game, seat);
  if (error) return { legal: false, reason: error };
  if (game.phase === 'final-guesses' || regularActionsUsed(game) >= QUESTION_LIMIT) return { legal: false, reason: 'question-limit' };
  ...
  game.moveCount += 1;
  return { legal: true, questionsRemaining: QUESTION_LIMIT - regularActionsUsed(game) };
}
...
function submitGuess(game, seat, guess) {
  const error = canTakeTurn(game, seat);
  if (error) return { legal: false, reason: error };
  const final = game.phase === 'final-guesses';
  if (!final && regularActionsUsed(game) >= QUESTION_LIMIT) return { legal: false, reason: 'question-limit' };
  if (!checkText(guess, 100)) return { legal: false, reason: 'invalid-guess' };
  game.pendingGuess = { seat: String(seat), text: guess.trim(), final };
  game.phase = 'judging';
  game.moveCount += 1;
  return { legal: true, questionsRemaining: QUESTION_LIMIT - regularActionsUsed(game) };
}
function voidRound(game, reason = 'drawer-timeout') {
  if (game.status !== 'playing') return { legal: false, reason: 'wrong-phase' };
  const result = {
    roundNumber: game.roundNumber, category: game.category, secret: game.secret,
    drawerSeat: game.drawerSeat, success: null, guesserSeat: null, winners: [],
    scores: { ...game.scores }, voided: true, reason,
  };
...
function judgeGuess(game, seat, correct) {
  ...
  if (guess.final) {
    ...
    game.turnIndex = (game.turnIndex + 1) % game.challengerSeats.length;
    if (regularActionsUsed(game) >= QUESTION_LIMIT) beginFinalGuesses(game);
    else game.phase = 'asking';
  }
...
function publicState(game) {
  return {
    ...
    questionsUsed: regularActionsUsed(game),
    questionsRemaining: Math.max(0, QUESTION_LIMIT - regularActionsUsed(game)),
...
module.exports = { QUESTION_LIMIT, ANSWERS, CATEGORIES, recommendedRounds, create, configure, beginRound,
  nextRound, setSecret, currentTurn, skipTurn, submitQuestion, answerQuestion, submitGuess, judgeGuess, voidRound,
