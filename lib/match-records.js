...
function validateMatch(match) {
  ...
  for (const item of match.outcomes) {
    ...
    if (item.rank !== undefined && !(Number.isInteger(item.rank) && item.rank >= 1 && item.rank <= 8)) throw new TypeError('Invalid player rank');
    ids.add(item.id);
  }
  return { id: match.id, gameType: match.gameType, at: match.at || new Date().toISOString(),
    outcomes: match.outcomes.map(({ id, result, rank }) => rank === undefined ? { id, result } : { id, result, rank }) };
}

// Land King (3-4 players) additionally reports each match's finish-order rank alongside the
// plain win/loss result, so a ranked game's per-game stats also expose an average finish place.
function tally(matches, playerId) {
  ...
  for (const match of matches) {
    ...
    for (const bucket of [total, byGame[match.gameType] ||= { played: 0, wins: 0, losses: 0, draws: 0, winRate: 0 }]) {
      ...
      if (Number.isInteger(item.rank)) {
        bucket.rankedMatches = (bucket.rankedMatches || 0) + 1;
        bucket.rankTotal = (bucket.rankTotal || 0) + item.rank;
      }
    }
  }
  for (const item of [total, ...Object.values(byGame)]) {
    item.winRate = item.played ? Math.round(item.wins / item.played * 10000) / 100 : 0;
    if (item.rankedMatches) { item.avgRank = Math.round(item.rankTotal / item.rankedMatches * 100) / 100; delete item.rankTotal; }
  }
  return { total, byGame };
}

// Only matches that were a direct two-person contest count toward head-to-head: a multi-seat
// game (bingo, pictionary, oldmaid, liar) or a 2v2 team match never has exactly two outcomes, so
// this naturally excludes them without needing to special-case any gameType.
function headToHead(matches, playerId, opponentId) {
  const total = { played: 0, wins: 0, losses: 0, draws: 0, winRate: 0 };
  const byGame = {};
  for (const match of matches) {
    if (!Array.isArray(match.outcomes) || match.outcomes.length !== 2) continue;
    const mine = match.outcomes.find(entry => entry.id === playerId);
    const theirs = match.outcomes.find(entry => entry.id === opponentId);
    if (!mine || !theirs) continue;
    for (const bucket of [total, byGame[match.gameType] ||= { played: 0, wins: 0, losses: 0, draws: 0, winRate: 0 }]) {
      bucket.played += 1;
      if (mine.result === 'win') bucket.wins += 1;
      if (mine.result === 'loss') bucket.losses += 1;
      if (mine.result === 'draw') bucket.draws += 1;
    }
  }
...
class JsonMatchStore {
  ...
  async headToHead(playerId, opponentId) {
    await this.queue;
    return headToHead(Object.values(this.matches), playerId, opponentId);
  }
}
...
class PostgresMatchStore {
  ...
  async headToHead(playerId, opponentId) {
    const rows = await this.pool.query(
      `SELECT game_type, outcomes FROM game_match_history
       WHERE jsonb_array_length(outcomes) = 2 AND outcomes @> $1::jsonb AND outcomes @> $2::jsonb`,
      [JSON.stringify([{ id: playerId }]), JSON.stringify([{ id: opponentId }])]);
    return headToHead(rows.rows.map(row => ({ gameType: row.game_type, outcomes: row.outcomes })), playerId, opponentId);
  }
}
...
module.exports = { createMatchStore, JsonMatchStore, PostgresMatchStore, validateMatch, tally, headToHead };
