...
async function requestHandler(req, res) {
  ...
  if (pathname === '/health' && req.method === 'GET') {
    return sendJson(res, 200, { ok: true, rooms: rooms.size, sessions: sessions.size, games: listGames().map((g) => g.id), version: '1.6.60', time: nowIso() });
  }
...
async function main() {
  ...
  server.listen(PORT, HOST, () => console.log(`게임 서버 v1.6.60 실행: http://${HOST}:${PORT}`));
}
