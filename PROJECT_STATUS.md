...
- 기준 브랜치: `main`
- 현재 릴리스: `v1.6.38`
- 이전 운영 커밋: `56d973a` (v1.6.37)
- 최신 반영 커밋: (병합 후 갱신 예정) (v1.6.38)
...
- **v1.6.38 (현재) 윷놀이 던지기 애니메이션(고급 CSS/JS 연출)** — 아래 상세
...
## v1.6.38 확정 범위 (최신 작업 상세)

윷놀이 던지기에 애니메이션을 추가했다. 사용자가 A(CSS 2D)/B(이미지·스프라이트)/C(짧은 영상 또는 고급 연출) 중 C를 선택했고, 이 환경에는 영상 제작·렌더링 도구가 없어 "고급 CSS/JS 애니메이션"으로 구현하기로 재확인했다. 게임 로직(`lib/games/yut.js`)은 전혀 건드리지 않았다.

### 1. 4-스틱 3D 입체 던지기 애니메이션 (`public/app.js`, `public/styles.css`, `public/index.html`)

랜드킹의 3D 주사위 큐브(v1.6.37)와 같은 패턴 — 윷가락 4개를 각각 앞면(평평한 면)·뒷면(둥근 면) 고정 마크업(`.yutStick`, 면 내용은 절대 바뀌지 않음)으로 두고, 던지는 동안은 각 스틱의 `transform`만 프레임마다 회전·상하 요동시키다가(`animateYutThrow()`), 멈출 때 서버가 확정한 "뒷면이 위로 나온 개수"(`g.lastThrow.backs`, 0~4)에 맞는 정확한 회전값으로 스냅한다. 어떤 스틱이 구체적으로 앞/뒤인지는 서버가 알려주지 않는(개수만 판정에 쓰이는) 정보라 화면 표시용으로만 무작위 배정했다 — 게임 판정에는 전혀 관여하지 않는다. 도·개·걸·윷·모·빽도 전부 이 개수 하나로 구분되며, 빽도는 도와 스틱 패턴이 같아 구분이 안 되므로 해당 스틱에 작은 강조 표시만 얹었다(장식일 뿐 판정과 무관).

### 2. 애니메이션 위치와 안정성

- 애니메이션은 공유 `#canvasWrap` 내부의 `#yutThrowStage` 오버레이로, 항상 게임판 중앙에서 재생된다(`pointer-events:none`이라 윷놀이는 애초에 보드 클릭을 쓰지 않으므로 다른 조작을 막지 않는다).
- 재접속 시 재생 방지: 랜드킹 주사위와 동일한 `yutThrowTrackingStarted` 키-비교 패턴(`yutLastThrowKey`)을 재사용해, 이미 있던 결과를 처음 볼 때는 애니메이션을 재생하지 않고 새 게임의 진짜 첫 던지기부터 정상 재생된다.
- 중복 던지기 방지: 버튼 클릭 즉시(`await roomAction` 이전) 비활성화하고, 애니메이션이 끝날 때까지(`yutThrowAnimating`) 서버 상태와 무관하게 추가로 비활성 상태를 유지한다.
- `prefers-reduced-motion`이면 회전 없이 바로 최종 모양으로 표시된다.
- 애니메이션은 약 1.2초 뒤 자동으로 사라지고, 결과 텍스트(`#yutLastThrow`)는 기존처럼 계속 남는다.
