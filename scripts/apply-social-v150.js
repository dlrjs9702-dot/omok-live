...
function replaceOnce(content, from, to, label) {
  ...
  if (content.indexOf(from, first + from.length) >= 0) throw new Error(`Patch target not unique: ${label}`);
  return content.slice(0, first) + to + content.slice(first + from.length);
}

const roomSocial = `'use strict';
...
`;
write('lib/room-social.js', roomSocial);

const roomSocialTest = `const test = require('node:test');
...
`;
write('test/room-social.test.js', roomSocialTest);
...
server = server.replaceAll("version: '1.4.2'", "version: '1.5.0'");
server = server.replaceAll('오목 서버 v1.4.2 실행', '게임 서버 v1.5.0 실행');
write('server.js', server);

let index = read('public/index.html');
index = index.replaceAll('v=1.4.2', 'v=1.5.0');
index = replaceOnce(index, '<label for="guestLabelInput">파일 이름</label>\n            <input id="guestLabelInput" autocomplete="off" maxlength="40" placeholder="예: 우성_PC" required />', '<label for="guestLabelInput">닉네임</label>\n            <input id="guestLabelInput" autocomplete="off" maxlength="40" placeholder="예: 우성" required />', 'index guest nickname field');
index = replaceOnce(index, '사람·기기별로 파일을 따로 발급하세요. 파일 자체가 사이트 출입 열쇠이며, 파일 1개는 동시에 1명만 사용할 수 있습니다.', '사람별 닉네임으로 입장 파일을 발급하세요. 이 닉네임은 접속자·플레이어·채팅 이름으로 사용되며, 파일 1개는 동시에 1명만 사용할 수 있습니다.', 'index admin description');
...
`, 'app player nickname + social renderers');
...
`  issueFileForm.addEventListener('submit', issueFile);
  ...
  copyRoomCodeBtn.addEventListener('click', copyRoomCode);
`, 'app chat event binding');
write('public/app.js', app);

let css = read('public/styles.css');
css = replaceOnce(css, 'grid-template-columns:minmax(0,1fr) 300px', 'grid-template-columns:minmax(0,1fr) 350px', 'css sidebar width');
...
console.log('Applied social/chat upgrade v1.5.0');
